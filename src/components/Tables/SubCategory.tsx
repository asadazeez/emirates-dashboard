...
const SubCategory = ({ subcategoryList }: Props) => {
  const [itemId, setItemId] = useState();

  const router = useRouter();


     async function featured(subCategoryId: any) {
       try {
         const featured: any = await subcategoryApi.featuredSubCategory(subCategoryId);

         if (featured.data.success) {
           toast.success(featured.data.message);
           router.refresh();
         }
       } catch (errors: any) {
  ...
  async function deleteSubCategory(subCategoryId: any) {
    try {
      const deleteSubCategory =
        await subcategoryApi.deleteSubCategory(subCategoryId);
      if (deleteSubCategory.data.success) {
        toast.success(deleteSubCategory.data.message);
        router.refresh();
      }
      if (!deleteSubCategory.data.success) {
        toast.error(deleteSubCategory.data.message);
      }
    } catch (errors: any) {
      toast.error(errors.message);
    }
    ...
    <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:py-7.5">
      ...
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
            {subcategoryList.map((Item: any, index: any) => (
              <tr key={index}>
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pl-7.5 ${index === Item.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <h5 className="text-dark dark:text-white">{Item.name}</h5>
                </td>
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 ${index === Item.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <p className="line-clamp-3 text-ellipsis text-dark dark:text-white">
                    {Item.description}
                  </p>
                </td>
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pl-7.5 ${index === subcategoryList.length - 1 ? "border-b-0" : "border-b"}`}
                >
                ...
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    <button className="hover:text-primary">
                      <Link href={`/admin/sub-categories/${Item._id}`}>
                        <svg
                    ...
                    >
                      {itemId === Item._id && (
                        <DeleteConfirmation
                          deleteId={deleteSubCategory}
                          id={Item._id}
                          isOpen={itemId === Item._id}
                          setIsOpen={setItemId}
                        />
                      )}
                      <svg
