 ...
 const Product = ({ productList }: Props) => {
   ...
   async function offerProduct(productId: any) {
     try {
       const offerProduct: any = await productApi.offerProduct(productId);

       if (offerProduct.data.success) {
         toast.success(offerProduct.data.message);
         router.refresh();
       }
     } catch (errors: any) {
                   ...
                   <label className="flex cursor-pointer select-none items-center">
                     <div className="relative">
                       <input
                         ...
                         className="sr-only"
                         onChange={async () => {
                           await offerProduct(productItem._id);
                         }}
                       />
                 ...
                 >
                   <div className="flex items-center justify-end space-x-3.5">
                     ...
                     <button className="hover:text-primary">
                       <Link href={`/admin/products/${productItem._id}`}>
                         <svg
                           xmlns="http://www.w3.org/2000/svg"
                           width="20"
                           height="20"
